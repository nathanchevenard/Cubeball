...
func initialize_teams(level : Level):
	...
	for team in team_list:
		...
		for i in team_size:
			...
			var random_x : float = randf_range((-level.length / 2), (level.length / 2))
			var random_z : float = randf_range((-level.width / 2), (level.width / 2))
			cuboid.global_position = Vector3(random_x, 2, random_z)
